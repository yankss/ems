  ...
  <div>
    ...
    <!-- 面包屑导航区 -->
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>报废处理</el-breadcrumb-item>
        </el-breadcrumb>
        </div>
...
export default {
  created() {
    const _this = this
    this.$axios.get('http://localhost:8181/scrapform/findAll/1/2').then(function(resp) {
      console.log(resp)
  ...
  methods: {
    handleClick(row) {
      console.log(row);
      const _this = this
      this.$axios.delete('http://localhost:8181/scrapform/deleteById/'+row.formId).then(function(resp) {
        console.log(resp)
        // location.reload();
      })
    },
    page(currentPage) {
      const _this = this
      this.$axios.get('http://localhost:8181/scrapform/findAll/'+currentPage+'/2').then(function(resp) {
        // console.log(resp)
        _this.tableData = resp.data.content
        _this.total = resp.data.totalElements
      })     
